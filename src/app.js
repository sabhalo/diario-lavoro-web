import { CHUNK_MS, DiaryStore, closeRecording, exportableChunks, formatTime, gapFor, id, isoNow, nextRecording, safeFileName, searchDocuments, sessionOffset, supersededAsrSegments, transcriptText } from "./core.js";
import { createZip, streamZip } from "./zip.js";

const TRANSFORMERS_URL = "https://cdn.jsdelivr.net/npm/@huggingface/transformers@3.8.1";
const ASR_MODEL = "Xenova/whisper-tiny";
const state = { store: null, view: "home", selectedId: null, display: null, mic: null, displayInfo: null, tests: { system: null, mic: null }, meters: new Map(), recording: null, segmenters: [], capturing: false, stopping: false, asr: { status: "in attesa" }, asrPipeline: null, asrModule: null };
...
async function init() {
  ...
  try { state.store = await new DiaryStore().open(); state.asr = (await state.store.get("settings", "asr-settings")) || state.asr; await updateStorage(); await recoverInterrupted(); await render(); }
...
async function recoverInterrupted() {
  ...
  for (const session of sessions.filter((item) => item.state === "aperta")) {
    ...
    for (const recording of recordings.filter((item) => item.status === "in-corso")) {
      ...
      for (const chunk of chunks.filter((item) => item.status === "scritto" || item.status === "confermato")) {
        const playable = !!chunk.blob && await isPlayable(chunk.blob, chunk.stream);
        if (!playable) {
          chunk.status = "non verificabile"; chunk.verifiedAt = isoNow(); await state.store.put("chunks", chunk);
          await state.store.put("gaps", { id: id("gap"), sessionId: session.id, recordingId: recording.id, startMs: chunk.startMs, endMs: chunk.endMs, cause: "blocco non recuperabile alla riapertura", certainty: "misurata", createdAt: isoNow() });
        } else if (chunk.status === "scritto") { chunk.status = "confermato"; chunk.verifiedAt = isoNow(); await state.store.put("chunks", chunk); }
      }
      ...
      const lastEnd = exportableChunks(chunks).reduce((latest, chunk) => Math.max(latest, chunk.endMs), recording.offsetStartMs);
      if (closed.offsetEndMs > lastEnd) await state.store.put("gaps", { id: id("gap"), sessionId: session.id, recordingId: recording.id, startMs: lastEnd, endMs: closed.offsetEndMs, cause: "intervallo dopo chiusura non acquisito", certainty: "incerta", createdAt: isoNow() });
...
async function renderCapture() {
  ...
  <section class="card"><h2>Trascrizione locale</h2><p id="asr-status">${renderAsrStatus(session)}</p><p class="small muted">Su gesto esplicito scarica Transformers.js 3.8.1 da jsDelivr e il modello pubblico ${ASR_MODEL} da Hugging Face (circa 105 MB osservati in sviluppo; misurare sul target). Audio e testo non vengono caricati: l’inferenza avviene nel browser. Dopo la cache, disconnetti la rete e usa “Verifica cache offline”.</p><div class="actions"><button class="secondary compact" data-action="prepare-asr" ${state.asr.status === "download in corso" ? "disabled" : ""}>Scarica e prepara ASR locale</button><select id="asr-source"><option value="microfono">Blocchi microfono confermati</option><option value="display">Blocchi audio computer confermati</option></select><button class="secondary compact" data-action="transcribe" ${state.asr.status !== "pronto" ? "disabled" : ""}>Trascrivi blocchi confermati</button><button class="secondary compact" data-action="verify-asr-offline" ${state.asr.status !== "pronto" ? "disabled" : ""}>Verifica cache offline</button></div><form data-form="transcript"><label>Segmento temporizzato (testo riconosciuto o correzione)<textarea name="text" required placeholder="Testo"></textarea></label><div class="grid"><label>Inizio (secondi)<input name="start" type="number" min="0" step="0.1" required></label><label>Fine (secondi)<input name="end" type="number" min="0" step="0.1" required></label></div><label>Provenienza<select name="source"><option value="manuale-locale">correzione manuale locale</option><option value="asr-locale-importato">ASR locale importato</option></select></label><button class="secondary">Salva segmento</button></form></section>
...
function renderAsrStatus(session) { const detail = state.asr.detail ? ` ${esc(state.asr.detail)}` : ""; return `Stato sessione: <strong>${esc(session.asrStatus || "in attesa")}</strong>. Motore: <strong>${esc(state.asr.status || "in attesa")}</strong>.${detail}`; }
...
async function onClick(event) {
  ...
  if (action === "prepare-asr") return prepareAsr();
  if (action === "transcribe") return transcribeSession();
  if (action === "verify-asr-offline") return verifyAsrOffline();
...
async function onSubmit(event) {
  ...
  if (form.dataset.form === "transcript") { const startMs = Number(data.get("start")) * 1000, endMs = Number(data.get("end")) * 1000, source = data.get("source"); if (endMs < startMs) return showNotice("La fine del segmento deve seguire l’inizio.", "danger"); const originals = (await state.store.bySession("transcripts", session.id)).filter((item) => item.source === "asr-locale" && item.active !== false && item.startMs <= startMs && item.endMs >= endMs).sort((a, b) => a.createdAt.localeCompare(b.createdAt)); const original = source === "manuale-locale" ? originals.at(-1) : null; if (original) { original.active = false; await state.store.put("transcripts", original); } await state.store.put("transcripts", { id: id("asr"), sessionId: session.id, recordingId: original?.recordingId ?? state.recording?.id ?? null, startMs, endMs, text: data.get("text").trim(), source, engine: source === "manuale-locale" ? "persona" : "ASR locale importato", parentId: original?.id ?? null, version: (original?.version || 0) + 1, active: true, createdAt: isoNow() }); session.asrStatus = "parziale"; session.updatedAt = isoNow(); await state.store.put("sessions", session); form.reset(); return renderCapture(); }
...
class Segmenter {
  constructor(kind, stream, recording, session) { this.kind = kind; this.stream = stream; this.recording = recording; this.session = session; this.active = false; this.recorder = null; this.timer = null; this.sequence = 0; this.settled = Promise.resolve(); }
  start() { this.active = true; this.begin(); }
  begin() { if (!this.active || !state.capturing) return; const startedAt = Date.now(), parts = []; let settle; this.settled = new Promise((resolve) => { settle = resolve; }); try { this.recorder = new MediaRecorder(this.stream); } catch (error) { settle(); failCapture(`MediaRecorder ${this.kind}: ${error.message}`); return; } const recorder = this.recorder; recorder.ondataavailable = (event) => event.data.size && parts.push(event.data); recorder.onerror = (event) => { failCapture(`MediaRecorder ${this.kind}: ${event.error?.name || "errore"} ${event.error?.message || ""}`); }; recorder.onstop = async () => { try { const endedAt = Date.now(), blob = new Blob(parts, { type: recorder.mimeType }); if (blob.size) await saveBlock(this, blob, startedAt, endedAt); else await failCapture(`MediaRecorder ${this.kind}: blocco vuoto`); } finally { settle(); if (this.active && state.capturing) this.begin(); } }; recorder.start(); this.timer = window.setTimeout(() => this.stop(), CHUNK_MS); }
  stop() { window.clearTimeout(this.timer); if (this.recorder?.state !== "inactive") this.recorder.stop(); }
  async finish() { this.active = false; this.stop(); await this.settled; }
}
...
async function runSearch(data) { const [sessions, notes, events, transcripts] = await Promise.all([state.store.all("sessions"), state.store.all("notes"), state.store.all("events"), state.store.all("transcripts")]); const from = data.get("from") ? new Date(data.get("from")).getTime() : 0, to = data.get("to") ? new Date(`${data.get("to")}T23:59:59`).getTime() : Infinity; const visible = sessions.filter((session) => { const time = new Date(session.startedAt).getTime(); return time >= from && time <= to; }); const hits = searchDocuments(data.get("query"), { sessions: visible, notes, events, transcripts: transcripts.filter((item) => item.active !== false) }).filter((hit) => visible.some((session) => session.id === hit.sessionId)); const target = byId("results"); target.innerHTML = hits.length ? hits.map((hit) => `<article class="result"><span class="chip">${esc(hit.kind)}</span><p>${esc(hit.text)}</p><button class="secondary compact" data-action="open-session" data-id="${hit.sessionId}">${hit.offsetMs != null ? `Apri a ${formatTime(hit.offsetMs)}` : "Apri sessione"}</button></article>`).join("") : `<p class="muted">Nessun risultato.</p>`; }
async function exportSession() { return exportScope(); }
async function exportScope(recordingId = null) { const picker = window.showSaveFilePicker?.({ suggestedName: "diario-export.zip", types: [{ description: "Archivio Diario", accept: { "application/zip": [".zip"] } }] }); const session = await selectedSession(), stem = `${safeFileName(session.title)}${recordingId ? `-${recordingId.slice(-6)}` : ""}`, suggested = `${stem}-export.zip`; let writable = null; if (picker) { try { writable = await (await picker).createWritable(); } catch (error) { if (error.name === "AbortError") return showNotice("Export annullato: nessun file creato."); showNotice(`Destinazione streaming non disponibile (${error.name}); preparo un archivio in memoria.`, "warn"); } } const filename = (chunk) => `media/${chunk.recordingId.slice(-8)}-${chunk.stream}-${chunk.index}.webm`; const [allRecordings, chunks, notes, events, gaps, allTranscripts] = await Promise.all([state.store.bySession("recordings", session.id), state.store.all("chunks"), state.store.bySession("notes", session.id), state.store.bySession("events", session.id), state.store.bySession("gaps", session.id), state.store.bySession("transcripts", session.id)]); const recordings = recordingId ? allRecordings.filter((item) => item.id === recordingId) : allRecordings, intervalStart = recordingId ? recordings[0]?.offsetStartMs : -Infinity, intervalEnd = recordingId ? recordings[0]?.offsetEndMs : Infinity; const allRelevant = chunks.filter((chunk) => chunk.sessionId === session.id && (!recordingId || chunk.recordingId === recordingId)).sort((a, b) => a.startMs - b.startMs), media = exportableChunks(allRelevant), transcripts = allTranscripts.filter((item) => item.startMs >= intervalStart && item.startMs < intervalEnd && item.active !== false), relevantGaps = gaps.filter((item) => item.startMs >= intervalStart && item.startMs < intervalEnd); const manifest = { schemaVersion: 1, exportedAt: isoNow(), scope: recordingId ? "singolo tratto" : "sessione integrale", session, recordings, notes: recordingId ? notes.filter((item) => item.startMs >= intervalStart && item.startMs < intervalEnd) : notes, events: recordingId ? events.filter((item) => item.startMs >= intervalStart && item.startMs < intervalEnd) : events, gaps: relevantGaps, transcripts, chunks: allRelevant.map(({ blob, ...metadata }) => ({ ...metadata, file: metadata.status === "confermato" ? filename(metadata) : null })), notices: ["Solo i blocchi con status confermato sono inclusi come media e dichiarati riproducibili.", "Lacune e trascrizione parziale restano nel manifest."] }; const entries = [{ name: "manifest.json", data: JSON.stringify(manifest, null, 2) }, { name: "trascrizione.txt", data: transcriptText(transcripts) }, ...media.map((chunk) => ({ name: filename(chunk), data: chunk.blob }))]; const size = media.reduce((total, chunk) => total + chunk.bytes, 0); try { if (writable) await streamZip(entries, writable); else { if (size > 300 * 1024 * 1024) throw new Error("Archivio oltre 300 MB: usa Chrome con il salvataggio streaming disponibile e scegli una destinazione locale."); download(await createZip(entries), suggested); } showNotice(`Export concluso: un archivio ZIP con manifest, testo e ${media.length} blocchi confermati. Riaprilo per verificare i file.`, "warn"); } catch (error) { try { await writable?.abort(); } catch {} showNotice(`Export non concluso: ${error.message}. I dati locali restano disponibili.`, "danger"); } }
...
async function setAsr(status, detail = "", extra = {}) { state.asr = { ...state.asr, ...extra, id: "asr-settings", status, detail, updatedAt: isoNow() }; await state.store.put("settings", state.asr); const session = await selectedSession(); const target = byId("asr-status"); if (target && session) target.innerHTML = renderAsrStatus(session); }
async function prepareAsr(cacheOnly = false) {
  if (state.asrPipeline) return state.asrPipeline;
  if (cacheOnly && navigator.onLine) throw new Error("Disconnetti la rete prima della verifica cache offline.");
  await setAsr(cacheOnly ? "verifica cache in corso" : "download in corso", cacheOnly ? "Ricerca esclusivamente file in cache locale." : "Scaricamento esplicito di runtime e modello in corso.");
  try {
    state.asrModule ||= await import(TRANSFORMERS_URL);
    state.asrModule.env.allowRemoteModels = !cacheOnly;
    const progress_callback = (progress) => { const text = progress.status === "progress" && progress.total ? `${progress.file || "modello"}: ${Math.round(progress.progress || 0)}%` : progress.status || "preparazione"; const target = byId("asr-status"); if (target) target.textContent = `ASR locale: ${text}`; };
    state.asrPipeline = await state.asrModule.pipeline("automatic-speech-recognition", ASR_MODEL, { dtype: "q8", progress_callback });
    await setAsr("pronto", cacheOnly ? "Cache offline verificata in questa sessione." : "Modello pronto in cache browser; nessun audio è stato inviato.", { model: ASR_MODEL, runtime: "Transformers.js 3.8.1", provider: "jsDelivr + Hugging Face", downloadedAt: state.asr.downloadedAt || isoNow(), cacheVerifiedAt: cacheOnly ? isoNow() : state.asr.cacheVerifiedAt || null }); await updateStorage();
    return state.asrPipeline;
  } catch (error) { state.asrPipeline = null; await setAsr("errore", `${error.name || "Errore"}: ${error.message || "preparazione ASR non riuscita"}`); throw error; }
  finally { if (state.asrModule) state.asrModule.env.allowRemoteModels = true; }
}
async function verifyAsrOffline() { try { state.asrPipeline = null; await prepareAsr(true); showNotice("Cache ASR verificata senza rete in questa sessione."); } catch (error) { showNotice(error.message, "warn"); } }
async function decodeTo16k(blob) { const context = new AudioContext(); try { const decoded = await context.decodeAudioData(await blob.arrayBuffer()); const offline = new OfflineAudioContext(1, Math.ceil(decoded.duration * 16_000), 16_000); const source = offline.createBufferSource(); source.buffer = decoded; source.connect(offline.destination); source.start(); const rendered = await offline.startRendering(); return rendered.getChannelData(0).slice(); } finally { await context.close(); } }
function asrChunks(result, block) { const duration = Math.max(0, block.endMs - block.startMs), raw = result.chunks?.length ? result.chunks : [{ text: result.text || "", timestamp: [0, duration / 1000] }]; return raw.filter((item) => item.text?.trim()).map((item) => { const [rawStart, rawEnd] = item.timestamp || [], start = Number.isFinite(rawStart) ? rawStart : 0, end = Number.isFinite(rawEnd) ? rawEnd : duration / 1000, startMs = block.startMs + Math.round(Math.max(0, Math.min(start, duration / 1000)) * 1000), endMs = block.startMs + Math.round(Math.max(Math.max(0, Math.min(start, duration / 1000)), Math.min(end, duration / 1000)) * 1000); return { startMs, endMs, text: item.text.trim() }; }).filter((item) => item.endMs >= item.startMs && item.endMs <= block.endMs); }
async function transcribeSession() {
  const session = await selectedSession(), source = byId("asr-source")?.value || "microfono"; if (!session) return;
  try {
    const pipeline = await prepareAsr(); const chunks = exportableChunks((await state.store.all("chunks")).filter((item) => item.sessionId === session.id && item.stream === source));
    if (!chunks.length) return showNotice(`Non ci sono blocchi ${source} confermati da trascrivere.`, "warn");
    session.asrStatus = "in elaborazione"; session.updatedAt = isoNow(); await state.store.put("sessions", session); await setAsr("in elaborazione", `${chunks.length} blocchi ${source} in coda.`); const prior = supersededAsrSegments(await state.store.bySession("transcripts", session.id), source); for (const item of prior) { item.active = false; await state.store.put("transcripts", item); }
    const runId = id("asr-run"), failures = [];
    for (let index = 0; index < chunks.length; index += 1) { const block = chunks[index]; await setAsr("in elaborazione", `Blocco ${index + 1}/${chunks.length} · ${formatTime(block.startMs)}.`); try { const audio = await decodeTo16k(block.blob); const result = await pipeline(audio, { language: "italian", task: "transcribe", return_timestamps: true, chunk_length_s: 10, stride_length_s: 0 }); for (const segment of asrChunks(result, block)) await state.store.put("transcripts", { id: id("asr"), sessionId: session.id, recordingId: block.recordingId, blockId: block.id, ...segment, source: "asr-locale", asrSource: source, engine: "Transformers.js 3.8.1", model: ASR_MODEL, runId, version: 1, active: true, createdAt: isoNow() }); } catch (error) { failures.push(`${formatTime(block.startMs)}: ${error.name || "errore"}`); } }
    session.asrStatus = failures.length ? "parziale" : "completa"; session.asrCoverage = { ...(session.asrCoverage || {}), [source]: { blocks: chunks.length, failed: failures.length, runId, updatedAt: isoNow() } }; session.updatedAt = isoNow(); await state.store.put("sessions", session); await setAsr("pronto", failures.length ? `Trascrizione parziale; audio conservato. ${failures.join(", ")}` : `Trascrizione completa di ${chunks.length} blocchi ${source}.`); renderCapture();
  } catch (error) { session.asrStatus = "errore"; session.updatedAt = isoNow(); await state.store.put("sessions", session); await setAsr("errore", `${error.name || "Errore"}: ${error.message || "ASR fallito; audio conservato."}`); renderCapture(); }
}
