import { CHUNK_MS, DiaryStore, closeRecording, exportableChunks, formatTime, gapFor, id, isoNow, nextRecording, safeFileName, searchDocuments, sessionOffset, transcriptText } from "./core.js";
...
function asrChunks(result, block) { const fallbackEnd = block.endMs - block.startMs; const raw = result.chunks?.length ? result.chunks : [{ text: result.text || "", timestamp: [0, fallbackEnd / 1000] }]; return raw.filter((item) => item.text?.trim()).map((item) => { const [start = 0, end = fallbackEnd / 1000] = item.timestamp || []; return { startMs: block.startMs + Math.round(start * 1000), endMs: block.startMs + Math.round(end * 1000), text: item.text.trim() }; }); }
async function transcribeSession() {
  ...
  try {
    ...
    session.asrStatus = "in elaborazione"; session.updatedAt = isoNow(); await state.store.put("sessions", session); await setAsr("in elaborazione", `${chunks.length} blocchi ${source} in coda.`); const prior = (await state.store.bySession("transcripts", session.id)).filter((item) => item.source === "asr-locale" && item.active !== false); for (const item of prior) { item.active = false; await state.store.put("transcripts", item); }
    const runId = id("asr-run"), failures = [];
    for (let index = 0; index < chunks.length; index += 1) { const block = chunks[index]; await setAsr("in elaborazione", `Blocco ${index + 1}/${chunks.length} · ${formatTime(block.startMs)}.`); try { const audio = await decodeTo16k(block.blob); const result = await pipeline(audio, { language: "italian", task: "transcribe", return_timestamps: true, chunk_length_s: 10, stride_length_s: 0 }); for (const segment of asrChunks(result, block)) await state.store.put("transcripts", { id: id("asr"), sessionId: session.id, recordingId: block.recordingId, blockId: block.id, ...segment, source: "asr-locale", engine: "Transformers.js 3.8.1", model: ASR_MODEL, runId, version: 1, active: true, createdAt: isoNow() }); } catch (error) { failures.push(`${formatTime(block.startMs)}: ${error.name || "errore"}`); } }
    session.asrStatus = failures.length ? "parziale" : "completa"; session.asrCoverage = { source, blocks: chunks.length, failed: failures.length, runId, updatedAt: isoNow() }; session.updatedAt = isoNow(); await state.store.put("sessions", session); await setAsr("pronto", failures.length ? `Trascrizione parziale; audio conservato. ${failures.join(", ")}` : `Trascrizione completa di ${chunks.length} blocchi ${source}.`); renderCapture();
