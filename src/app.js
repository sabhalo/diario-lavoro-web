import { captureIsLive, CHUNK_MS, continuousBlockInterval, DiaryStore, closeRecording, exportableChunks, formatTime, gapAfterConfirmed, id, isoNow, mediaFileName, nextRecording, overlapsScope, recordingExportScope, safeFileName, searchDocuments, sessionOffset, storageAdmission } from "./core.js?v=11";
...
async function recoverInterrupted() {
  ...
  for (const session of sessions.filter((item) => item.state === "aperta")) {
    ...
    for (const recording of recordings.filter((item) => item.status === "in-corso")) {
      ...
      const gap = gapAfterConfirmed(closed, chunks, "intervallo dopo chiusura non verificabile", "incerta");
...
async function renderVerification() {
  view.innerHTML = `<header><div><h1>Verifica e limiti</h1><p class="muted">Lo sviluppo locale non sostituisce le prove sulla build Chrome/macOS.</p></div></header><section class="card"><h2>Stato onesto della consegna</h2><ul class="checklist"><li><strong>AC1–AC2, preflight dei tre flussi:</strong> esito breve positivo solo riferito dall’utente; non osservato dall’agente né misurato sul profilo target.</li><li><strong>Cattura continua:</strong> questa build elimina lo stop/start fra frammenti; continuità, codec e riproduzione restano da misurare sul Mac target.</li><li><strong>AC3–AC6 e AC8–AC10:</strong> ancora da provare sulla build e sul Mac target, incluse due ore, offline, quota, crash/revoca/sleep e riapertura export.</li><li><strong>Uso reale con dati aziendali o persone:</strong> bloccato dal gate policy, ancora senza evidenza aziendale.</li></ul><p>Questa build registra solo video e audio; trascrizione e ASR sono fuori dal runtime. Il preflight controlla selezione monitor e presenza delle tracce; un indicatore live non è prova di contenuto.</p><p><a href="docs/verification/capture-preflight-mac-reported-2026-09-22.md" target="_blank">Resoconto della prova breve riferita</a> · <a href="docs/verification/acceptance-matrix-2026-09-22.md" target="_blank">Matrice dei criteri di accettazione</a></p></section>`;
}
...
async function saveBlock(segmenter, blob, startMs, endMs) {
  const item = { id: id("block"), recordingId: segmenter.recording.id, sessionId: segmenter.session.id, index: segmenter.sequence++, stream: segmenter.kind, startMs, endMs, format: blob.type || "video/webm", bytes: blob.size, status: "scritto", verifiedAt: null, blob, createdAt: isoNow() };
