...
async function prepareBrowser() {
  ...
  try {
    ...
    const pipe = await loadWhisper(tier.model, { device: tier.device, dtype: tier.dtype, progress_callback: (progress) => { const done = Number(progress.progress); if (Number.isFinite(done)) { state.asr.loading = `Modello ${Math.round(done)}%`; const button = document.querySelector('[data-action="prepare-browser"]'); if (button) button.textContent = state.asr.loading; } } });
