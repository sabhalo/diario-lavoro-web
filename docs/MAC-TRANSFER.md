...
Aprire `http://127.0.0.1:4173/` in Chrome. Per una copia già clonata: entrare nella cartella, passare a `main` con `git switch main`, usare `git pull --ff-only` e riavviare il server. Fermarlo con `Ctrl+C` quando non serve più.
