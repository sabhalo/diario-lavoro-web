...
3. Avviare `python3 -m http.server 4173 --bind 127.0.0.1` e aprire `http://127.0.0.1:4173/` in Chrome. Se si sostituisce una build precedente nello stesso profilo e resta “inizializzazione”, ricaricare la pagina una volta dopo l'aggiornamento del service worker. Fermare con `Ctrl+C` alla fine.
...
- AC7: positivo solo secondo il riscontro più recente dell’utente; annotare profilo, testo, timestamp/qualità/tempo/RAM se si ripete la prova con frase innocua.
- AC8: offline dopo cache esplicita, con rete scollegata, e audio integro in errore ASR.
